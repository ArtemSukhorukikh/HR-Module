...
class CompetenceRequest extends AbstractRequestDTOTransformer
{
    ...
    public function transformToObject($object): Competence
    {
        ...
        foreach($object->competences_id as $i){
            $data->addCompetence($this->competenceRepository->find($i));
        }
        foreach($object->includes_id as $i){
            $data->addInclude($this->competenceRepository->find($i));
        }
        foreach($object->skills_id as $i){
            $data->addSkill($this->skillsRepository->find($i));
        }
        foreach($object->users_id as $i){
            $data->addUser($this->userRepository->find($i));
        }
        foreach($object->educational_resources as $i){
            $data->addEducationalResource($this->educationalResourcesRepository->find($i));
        }
        return $data;
    }
}